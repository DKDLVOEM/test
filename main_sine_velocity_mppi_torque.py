...
class Args:
    ...
    mppi_num_samples: int = 128
    # mppi_lambda: float = 1.0  # temperature
    mppi_lambda: float = 0.05  # temperature
    # mppi_noise_sigma: float = 0.02  # base sigma for xyz / orient; gripper noise scaled down
    mppi_noise_sigma: float = 0.1  # base sigma for xyz / orient; gripper noise scaled down
    w_ref: float = 1.0
    # w_goal: float = 5.0
    w_goal: float = 5.0
    w_reg: float = 0.1
    ...
    ee_body_name: str = "gripper0_eef"  # end-effector body/site name for FK/Jacobian
    tau_kp: float = 200.0
    tau_kd: float = 10.0
    ...
    def plan(
        ...
        for j in range(self.num_samples):
            ...
            for k in range(K):
                ...
                if (j == 0):
                    print(k, " th")
                    predicted_delta_p = p_k - p_obj_pred
                    print("predicted delta x: ", predicted_delta_p[0])
                    print("predicted delta y: ", predicted_delta_p[1])
                    print("predicted delta z: ", predicted_delta_p[2])
        ...
        # for debug
        A_ref_unscaled = action_unscale(A_ref[:1][0])
        u0_new_unscaled = action_unscale(u0_new)

        print("original input:", A_ref_unscaled[:3])
        print("final input:", u0_new_unscaled[:3])
        # orientation fix...
        original_input = A_ref[:1][0]
        u0_new[3:] = original_input[3:]
        return u0_new

    def _dynamics_step(self, x: np.ndarray, u: np.ndarray) -> np.ndarray:
        """x_{k+1} = [p + dp, aa + daa, g_cmd]"""
        out = x.copy()
        u_unscaled = action_unscale(u)
        out[:3] = x[:3] + u_unscaled[:3]
        out[3:6] = x[3:6] + u_unscaled[3:6]
        out[6] = u[6]
        return out

    def _compute_phase(self, d: float, g_cmd: float) -> str:
        if d <= self.d_close or g_cmd > 0.0:
            return "grasp"
        if d <= self.d_far:
            return "near"
        return "approach"
...
class JointTorqueMujocoMPPIController:
    ...
    def _build_eef_ref(self, p_start: np.ndarray, dp_seq: np.ndarray) -> np.ndarray:
        ...
        acc = p_start.copy()
        for k in range(K):
            acc = acc + action_unscale_3d(dp_seq[k])
            prefs[k] = acc
        return prefs
    ...
    def _build_tau_ref(self, q0_arm: np.ndarray, qdot0_arm: np.ndarray, A_ref: np.ndarray) -> np.ndarray:
        ...
        qdot = qdot0_arm.copy()
        for k in range(K):
            dp = action_unscale_3d(A_ref[k, :3])
...
def eval_libero(args: Args) -> None:
    ...
    if args.visualize:
        try:
            ...
            logging.warning(f"visualize-True but failed to import cv2: {e}. Disabling visualization.")
